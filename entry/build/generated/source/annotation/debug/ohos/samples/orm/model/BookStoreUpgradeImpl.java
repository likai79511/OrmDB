...
/**
 * Generated by ohos.data.orm.processor for ohos.samples.orm.model.BookStoreUpgrade. Do not modify!
 *
 * @author Huawei
 * @since 2021-07-20
 */
public class BookStoreUpgradeImpl extends BookStoreUpgrade {
    private OrmMigration[] upgradeMigrations;

    private OrmMigration[] downgradeMigrations;

    public BookStoreUpgradeImpl() {
        super();
        this.upgradeMigrations = null;
        this.downgradeMigrations = null;
    }

    public BookStoreUpgradeImpl(OrmMigration[] upgradeMigrations,
            OrmMigration[] downgradeMigrations) {
        super();
    ...
    public RdbOpenCallback getHelper() {
        return new RdbOpenCallback() {
            ...
            public void onCreate(RdbStore store) {
                store.executeSql("CREATE TABLE IF NOT EXISTS `BookUpgrade` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `Name` TEXT , `user_id` INTEGER , `addColumn12` INTEGER , FOREIGN KEY (`user_id`) REFERENCES `user` (`userId`) ON UPDATE NO ACTION ON DELETE CASCADE)");
                store.executeSql("CREATE INDEX `index_BookUpgrade_Name` ON `BookUpgrade` (`Name`)");
